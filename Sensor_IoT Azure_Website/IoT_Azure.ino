...
void setup()
{
  ...
  myServo.attach(17);       // Hubungkan servo ke pin 17
}
...
void loop()
{
 // Servo control based on distance
  if (distance < 50) {
    myServo.write(90);  // Move to 90 degrees if object is within 50 cm
    Serial.println("(ON)");
  } else {
    myServo.write(0);  // Move back to 0 degrees if object is not within 50 cm
    Serial.println("(OFF)");
  }
